  ...
  if (!user) {
    return (
      <button className="fr-login" onClick={onSignIn}>
        ...
        <span>Login</span>
        <style>{`
          .fr-login { display: inline-flex; align-items: center; gap: var(--fr-s2); height: 44px; padding: 0 var(--fr-s4); background: none; border: 1px solid var(--fr-line-strong); border-radius: var(--fr-r-pill); font-family: var(--fr-font-sans); font-size: 0.88rem; font-weight: 600; color: var(--fr-text); cursor: pointer; white-space: nowrap; transition: border-color var(--fr-dur-quick) var(--fr-ease-standard), color var(--fr-dur-quick) var(--fr-ease-standard); }
  ...
  return (
    <div className="fr-usermenu">
      ...
      <style>{`
        ...
        .fr-avatar-fallback { background: var(--fr-brand-tint); color: var(--fr-brand); font-weight: 700; font-size: 1rem; }
        .fr-dropdown { position: absolute; top: calc(100% + var(--fr-s2)); right: 0; min-width: 184px; background: var(--fr-surface); border: 1px solid var(--fr-line); border-radius: var(--fr-r-card); box-shadow: var(--fr-elev-2); padding: var(--fr-s2); z-index: var(--fr-z-dropdown); }
        .fr-dropdown-item { display: block; width: 100%; text-align: left; padding: var(--fr-s3); border: none; background: none; border-radius: var(--fr-r-control); font-family: var(--fr-font-sans); font-size: 0.9rem; color: var(--fr-text); text-decoration: none; cursor: pointer; transition: background var(--fr-dur-quick) var(--fr-ease-standard); }
